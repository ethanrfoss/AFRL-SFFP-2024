...
MoonJ2.SetField("CentralBody","Luna");
...
function xdot = Dynamics(x,t,ForceModel)
    ForceModelFailure = ForceModel.GetDerivatives(x,t);
    if ~ForceModelFailure
        error('Force Model Failed!');
    end
    xdot = ForceModel.GetDerivativeArray();
end
...
tspan = [0 33*24*60*60];
[t,x] = ode89(@(t,x) Dynamics(x,t,MoonJ2),tspan,Propagator.GetState(),odeset("RelTol",10^-14,"AbsTol",10^-14,"InitialStep",60));
